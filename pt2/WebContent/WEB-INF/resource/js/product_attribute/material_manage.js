...
function bodyLoad() {
	$.ajax({
		type : "GET",
		url : baseURL + "/product_attribute/get_all_material",
 ...
 *            从服务器返回的JSON数据
 */
function showMaterials(mtJsonArray) {
	// 结果信息对象
	var joRe = eval("(" + mtJsonArray + ")");
//	var joRe = mtJsonArray;
...
function filTable(exportField, exportContent) {
	...
	for (var i = 0; i < exportContent.length; i++) {
		...
		// 为每条记录添加删除链接
		var delHref = baseURL + "/product_attribute/delete_material?mtId=" + jsonObj[exportField[0][0]];
